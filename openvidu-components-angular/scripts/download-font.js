...
const CSS_FILE_PATH = path.join(__dirname, `../dist/openvidu-webcomponent/openvidu-webcomponent-${ VERSION }.css`)
const distPath = path.join(__dirname, '../dist/openvidu-webcomponent/fonts')
...
const downloadUrls = (urls) => {
  return Promise.all(urls.map(async url => {
    ...
    return {
      oldUrl: url,
      newUrl: path.relative(path.join(__dirname, '../dist/openvidu-webcomponent'), newUrl)
    }
  }))
}
